...
#' @param version \emph{integer.} The Stata dta file version you like to write. Default 115 (Stata 12). Supported versions are 115, 114 and 113 coresponding to Stata 12, 10 and 8. You can add the correct format or may type Stata version and the correct format will be choosen.
...
save.dtaOld <- function(data, file="path", data.label=NULL, time.stamp=TRUE,
  ...
  if (!compress) {
    ...
    for (k in names(which(ff))) {
      vartypen[k][varTmin[k] < (-fminmax) | varTmax[k] > fminmax] <- 255
      vartypen[k][varTmin[k] > (-fminmax) & varTmax[k] < fminmax] <- 254
    }
    ...
    for (k in names(which(ii))) {
      vartypen[k][varTmin[k] < imin | varTmax[k] > imax] <- 253
      vartypen[k][varTmin[k] > imin & varTmax[k] < imax] <- 252
      vartypen[k][varTmin[k] > bmin & varTmax[k] < bmax] <- 251
    }

    factorlength <- sapply(data[factors], nlevels)
    for (k in names(which(factors))) {
      vartypen[factors & factorlength[k] > 0x1.000000p127] <- 253
      vartypen[factors & factorlength[k] < 0x1.000000p127] <- 252
      vartypen[factors & factorlength[k] < 101] <- 251
    }
  }
  ...
  formats <- vartypen
  formats[formats == -255] <- "%td"
  formats[formats == 255] <- "%9.0g"
  formats[formats == 254] <- "%9.0g"
  formats[formats == 253] <- "%9.0g"
  formats[formats == 252] <- "%9.0g"
  formats[formats == 251] <- "%9.0g"
  formats[vartypen >= 0 & vartypen < 244] <-
    paste0("%-", formats[vartypen >= 0 & vartypen < 244], "s")
